...
import { Check, X, Clock, ChevronLeft, ChevronRight, Save } from 'lucide-react'
import api from '../../api'
...
export default function Attendance() {
  ...
  const [attendance, setAttendance] = useState(() => {
    const init = {}
    days.forEach((d, i) => { init[i] = { breakfast: i === 0, lunch: false, dinner: true } })
    return init
  })
  ...
  const handleSave = async () => {
    // Send declarations to backend for all days
    try {
      const promises = []
      for (let i = 0; i < days.length; i++) {
        const d = days[i].full.toISOString().split('T')[0] // YYYY-MM-DD
